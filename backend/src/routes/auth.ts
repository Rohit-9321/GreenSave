...
// --- Register (tolerant + proper duplicate handling)
router.post('/register', async (req, res) => {
  try {
    ...
    const user = await prisma.user.create({
      data: { email, name: safeName, passwordHash: hash }
    })
...
router.post('/demo', async (_req, res) => {
  ...
  if (!user) {
    user = await prisma.user.create({
      data: { email: 'demo@greensteps.app', name: 'Demo User', passwordHash: bcrypt.hashSync('demo1234',10) }
    })
  }
  const token = signToken({ id: user.id, email: user.email, name: user.name })
  res.json({ token, user: { id: user.id, email: user.email, name: user.name } })
...
// --- OTP (optional; safe if SMTP unset)
router.post('/otp/start', async (req, res) => {
  ...
  if (SMTP.host && SMTP.user && SMTP.pass) {
    try {
      ...
      console.warn('SMTP send failed; continuing without email', e)
    }
  }
...
router.post('/otp/verify', async (req, res) => {
  ...
  if (!user) {
    user = await prisma.user.create({
      data: { email, name: name || email.split('@')[0], passwordHash: bcrypt.hashSync(Math.random().toString(36), 10) }
    })
  }
  const token = signToken({ id: user.id, email: user.email, name: user.name })
  res.json({ token, user: { id: user.id, email: user.email, name: user.name } })
